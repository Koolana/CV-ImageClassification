...
Для запуска обучения и проверки модели на датасете необходимо сначало скопировать датасет в папку **datasets**, так, чтобы каждая вложенная папка соответсвовала одному определенному классу, а затем запустить следующую команду из корня данного проекта:
